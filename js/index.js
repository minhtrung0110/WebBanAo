...
function renderProduct(arrival) {
    ...
    const content = arrival.map((item, index) => {
            ...
            html += '</p>';
            html += '<button type="button" class="btn btn-outline-warning" onclick="checkLogin()" >Mua Ngay</button> ';          
            html += '</div> ';
            html += '</div> ';
...
function renderBestseller(bestseller) {
    ...
    const content = bestseller.map((item, index) => {
            ...
            html += '<button type="button" class="btn btn-outline-warning col-md-4 " style="float: right;"><a onclick="checkLogin()">Mua Ngay</a> </button> ';
            html += '</div> </div>';
            html += '</div> ';
...
function checkLogin()
{
    alert("Bạn cần đăng ký thành viên trước khi mua hàng !");
}